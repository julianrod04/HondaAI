...
class DummyVecEnv(VecEnv):
	"""
	Dummy Class for evaluation, Gym need wrapper.
	Writes correct observation after Env is reset.
	...
	def __init__(self, env_fns: List[Callable[[], gym.Env]]):
		...
		self.reward_cnt = self.tmp_config.num_rewards + 1
		...
		self.buf_rews = np.zeros((self.num_envs, self.reward_cnt), dtype=np.float32)
	...
	def step_wait(self) -> VecEnvStepReturn:
		...
		for env_idx in range(self.num_envs):
			...
			if self.buf_dones[env_idx]:
				...
				#self.buf_infos[env_idx]["points_reached"] = self.buf_infos[env_idx]['points_reached']
	...
	def reset(self) -> VecEnvObs:
		for env_idx in range(self.num_envs):
			# only evaluation 
			obs, _ = self.envs[env_idx].reset(seed=self._seeds[env_idx])
			self._save_obs(env_idx, obs)
	...
	def _obs_from_buf(self) -> VecEnvObs:
		return dict_to_obs(self.observation_space, OrderedDict([(k, np.copy(v)) for k, v in self.buf_obs.items()]))
